...
- `.gitignore` excludes `.env` and `node_modules`; `.env.example` documents required vars.
...
**Verify checkpoint:**
- Client initializes from `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`.
...
- RLS is **enabled** with select/insert/update/delete policies all scoped to `auth.uid() = user_id`.
...
- Approving saves **only** the redacted text to `contracts.redacted_text`.
...
**Verify checkpoint:**
- Function reads `OPENAI_API_KEY` from the environment (never from the client).
...
- `important_deadlines` and `obligations` render as readable lists from their jsonb arrays.
...
- `VITE_SUPABASE_*` and `OPENAI_API_KEY` set in Netlify env.
...
- `README.md`
- `AGENTS.md`
- `PLAN.md` / `BUILD_STEPS.md` (final status pass)
